...
const VALID_STATUSES = ["Da Contattare", "Inviata Mail", "In Trattativa", "Vinto (Cliente)", "Perso"];
...
router.patch("/:id/status", (req, res) => {
  ...
  if (!VALID_STATUSES.includes(status)) {
    return res.status(400).json({ error: `Stato non valido. Valori ammessi: ${VALID_STATUSES.join(", ")}` });
  }
  const db = getDb();
  const old = db
